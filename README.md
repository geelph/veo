...
- 2026/06/15:
  ```
  - New：
   1、新增全局超时止损机制，目录扫描与主动指纹探测在连续超时或高比例超时时自动丢弃目标，避免大批量扫描被单个异常站点拖慢。
   2、新增运行时请求配置模块，统一管理线程数、重试次数、超时时间、代理与随机 UA 等扫描参数。

  - Bug Fix：
   1、修复主动指纹扫描在客户端 JS/HTML 自跳转或循环跳转场景下反复请求导致卡住的问题。
   2、修复请求超时只在返回后被丢弃、无法真正中断底层 HTTP 请求的问题，fasthttp 请求现在会按 Context Deadline 收敛。
   3、修复主动指纹探测阶段 context 未传递到 path/header/icon 请求的问题，避免单目标探测超出全局超时预算。
   4、修复重定向后最终 URL 未正确回写到响应结果的问题。

  - Optimize：
   1、统一主动指纹、被动指纹、目录扫描的默认请求超时时间为 3 秒，并统一超时规范化逻辑。
   2、优化主动指纹探测并发模型，收敛单目标内部请求并发，减少批量扫描时的请求放大。
   3、优化控制台输出格式，取消指纹识别与目录扫描结果中的 Content-Type 打印，标题统一高亮显示，指纹信息使用浅紫色显示。
   4、优化实时 CSV 与 JSON 报告输出，统一响应记录结构并补充指纹、时间戳、Host 等字段处理。
   5、优化被动代理插件接口，按事件拆分 addon 能力，降低无关方法实现成本。
   6、更新指纹规则与目录字典，清理部分敏感规则。
  ```
